...
parseExprNoSimplify :: CharParser st Expr
parseExprNoSimplify = ex1NoSimplify
...
ex1NoSimplify :: CharParser st Expr
ex1NoSimplify = ex2NoSimplify `chainl1` impop

ex2NoSimplify :: CharParser st Expr
ex2NoSimplify = ex3NoSimplify `chainl1` (op "||" >> return (\x y -> manyOrNoSimplify [x, y]))

ex3NoSimplify :: CharParser st Expr
ex3NoSimplify = ex4 `chainl1` (op "&&" >> return (\x y -> manyAndNoSimplify [x, y]))
...
integer = lexeme $ read <$> (plus <|> minus <|> number) -- read <$> many1 digit -- do { ds <- many1 digit ; return (read ds) }
    where plus   = char '+' *> number -- borrowed from https://www.schoolofhaskell.com/school/to-infinity-and-beyond/pick-of-the-week/parsing-floats-with-parsec
          minus  = (:) <$> char '-' <*> number
          number = many1 digit
...
pexpr :: QuasiQuoter
pexpr = QuasiQuoter { quoteExp = quoteFormExp parseExprNoSimplify
                    , quotePat = quoteFormPat parseExprNoSimplify
                    , quoteType = undefined
                    , quoteDec = undefined
                    }
